...
public class Funcs {
	...
	@Autowired
	private ResponseDTOs responseDTO;
	...
	public ResponseEntity<?> generateResponse(List<User> users, ResponseDto resp, HttpStatus status, String details){
		responseDTO.setTimestamp(LocalDateTime.now());
		responseDTO.setHttpStatusCode(status.value());
		responseDTO.setDetails(details);
		responseDTO.setUsers(users);
		responseDTO.setResponseDto(resp);
